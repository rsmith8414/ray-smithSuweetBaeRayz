...
document.addEventListener("DOMContentLoaded", () => {
    ...
    items.forEach(item => {
      ...
      card.innerHTML = `
        ...
        <p class="text-xs text-center font-bold mb-2 ${
          ...
          item.restaurant === "SushiLand" ? "text-blue-600" :
          item.restaurant === "SmitherBeans" ? "text-yellow-600" : "text-gray-400"
        }">${item.restaurant}</p>
        <div class="text-center">
          <button class="bg-lime-600 text-white px-4 py-1 rounded hover:bg-lime-700 transition add-to-cart" data-name="${item.name}" data-price="${item.price}" data-restaurant="${item.restaurant}">
            Add to Cart
          </button>
        </div>
      `;
  ...
  document.addEventListener("click", function (e) {
    if (e.target.classList.contains("add-to-cart")) {
      ...
      const restaurant = e.target.dataset.restaurant || "Unknown";
